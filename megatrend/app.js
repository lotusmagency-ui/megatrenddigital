...
const translations = {
  bg: {
    ...
    servicesTitle1: 'КАК',
    servicesTitle2: 'ГО ПРАВИМ',
    servicesSub: 'да постигнем резултати преди края на първата седмица',
    ...
    testTitle1: 'НОСЯТ СЕ',
    testTitle2: 'СЛУХОВЕ',
    testSub: 'От реални партньори, с които не можем да спорим',
  ...
  en: {
    ...
    servicesTitle1: 'KAK',
    servicesTitle2: 'GO PRAVIM',
    servicesSub: 'da postignem rezultati predi kraya na parvata sedmitsa',
    ...
    testTitle1: 'NOSYAT SE',
    testTitle2: 'SLUHOVE',
    testSub: 'Ot realni partnyori, s koito ne mozheme da sporim',
...
function initHeroAnimations() {
  ...
  gsap.set(heroTitleSpans, { y: '100%' });
  gsap.set(heroCta, { opacity: 0, scale: 0.9 });

  const tl = gsap.timeline({ delay: 0.5 });
  tl.to(heroTitleSpans, { y: '0%', duration: 1, stagger: 0.15, ease: 'power4.out' })
    .to(heroCta, { opacity: 1, scale: 1, duration: 0.6, ease: 'back.out(1.5)' }, '-=0.4');
}
      ...
      onEnter: () => {
        ...
        gsap.to(obj, {
          ...
          onUpdate: () => {
            ...
            el.innerHTML = `${stat.prefix}${val}<span>${stat.suffix}</span>`;
          }
        });
      }
    });
